...
func (c *Collection[T]) Collect() {
	...
	for id := range c.items {
		...
		c.lg.Append(fmt.Sprintf("image %d collection queued", idHere), log.LEVEL_INFO)
		c.Schedule(id, &Task[T]{
			Lib:  "internal",
			Name: "collect",
			Fn: func(i *Item[T]) {
				c.lg.Append(fmt.Sprintf("image %d collected", idHere), log.LEVEL_INFO)
				i.Self = nil
				i.cleaned = true
				wg.Done()
			},
	...
	wg.Wait()
	c.lg.Append("all images collected", log.LEVEL_INFO)
}
