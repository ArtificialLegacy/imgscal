			...
			gui.wg_column({
				gui.wg_align(gui.ALIGN_CENTER):to({
					gui.wg_style():set_style_float(gui.STYLEVAR_CHILDROUNDING, 10):to({
						gui.wg_child():size(200 + wpx * 2, 200 + wpy * 2):layout({
							...
							gui.wg_image_sync(img):size(200, 200),
						}),
					}),
				}),
				gui.wg_style():set_style_float(gui.STYLEVAR_CHILDROUNDING, 10):to({
					gui.wg_child():layout({
						gui.wg_tab_bar():tab_items({
							gui.wg_tab_item("simplex_2d"):layout({
								gui.wg_label(simplex2_desc):wrapped(true),
								gui.wg_separator(),
								gui.wg_spacing(),
								gui.wg_spacing(),
								wg_seed(seed, rnd, normal),
								widget.slider_float("Scale Factor:", scale, 0.01, 0.5, 0.25),
								gui.wg_row({
							...
							gui.wg_tab_item("simplex_3d"):layout({
								gui.wg_label(simplex3_desc):wrapped(true),
								gui.wg_separator(),
								gui.wg_spacing(),
								gui.wg_spacing(),
								wg_seed(seed, rnd, normal),
								widget.slider_float("Scale Factor:", scale, 0.01, 0.5, 0.25),
								widget.slider_float("Alpha Offset:", aoff, 0.1, 1.5, 0.5),
								wg_gen3(img, ready, seed, rnd, scale, aoff, normal),
							}),
							gui.wg_tab_item("simplex_4d"):layout({
								gui.wg_label(simplex4_desc):wrapped(true),
								gui.wg_separator(),
								gui.wg_spacing(),
								gui.wg_spacing(),
								wg_seed(seed, rnd, normal),
								widget.slider_float("Scale Factor:", scale, 0.01, 0.5, 0.25),
								widget.slider_float("Green Offset:", goff, 0.1, 1.5, 0.5),
								widget.slider_float("Blue Offset:", boff, 0.1, 1.5, 0.5),
...
function wg_gen2(img, ready, seed, rnd, scale, direct, ecolor, ealpha, normal)
	return gui.wg_align(gui.ALIGN_CENTER):to({
		gui.wg_button("Generate"):disabled(not ref.get(ready)):size(100, 50):on_click(function()
			genNoise2(
				img,
				ref.get(seed),
				ref.get(scale),
...
function wg_gen3(img, ready, seed, rnd, scale, aoff, normal)
	return gui.wg_align(gui.ALIGN_CENTER):to({
		gui.wg_button("Generate"):disabled(not ref.get(ready)):size(100, 50):on_click(function()
			genNoise3(img, ref.get(seed), ref.get(scale), ref.get(aoff), ref.get(normal), ready)
...
function wg_gen4(img, ready, seed, rnd, scale, goff, boff, normal)
	return gui.wg_align(gui.ALIGN_CENTER):to({
		gui.wg_button("Generate"):disabled(not ref.get(ready)):size(100, 50):on_click(function()
			genNoise4(img, ref.get(seed), ref.get(scale), ref.get(goff), ref.get(boff), ref.get(normal), ready)
