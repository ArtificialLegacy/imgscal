...
function main()
    ...
    local imgHistogram = image.new_filled("histogram", image.ENCODING_PNG, 256, 256, image.color_rgb(0, 0, 0), image.MODEL_RGBA)

    local wpx, wpy = gui.window_padding()
    local spx, spy = gui.item_spacing()
    local isx, isy = gui.item_inner_spacing()
        ...
        gui.window_single():layout({
            ...
            gui.wg_custom(function()
                if ref.get(histogramMode) == 1 then
                    gui.layout({gui.wg_align(gui.ALIGN_CENTER):to({
                        gui.wg_style()
                            :set_style_float(gui.STYLEVAR_CHILDROUNDING, 10)
                            :to({gui.wg_child()
                                :size((256 + wpx * 2), 256 + wpy * 2 + 4)
                                :layout({
                                    -- using wg_image_sync here allows it to display while the image is being processed.
                                    -- otherwise the main goroutine would be blocked here
                                    gui.wg_image_sync(imgHistogram)
                                        :size(256, 256),
                                }),
                            }),
                    })})
                            ...
                            :to({gui.wg_child()
                                ...
                                :layout({
                                    gui.wg_plot("Histogram")
                                        ...
                                        :y_axeflags(gui.FLAGPLOTAXIS_NOHIGHLIGHT, gui.FLAGPLOTAXIS_NOHIGHLIGHT, gui.FLAGPLOTAXIS_NOHIGHLIGHT)
                                        :plots({
                    ...
                    :on_click(function()
                        ...
                        if ref.get(paddingEnabled) then
                            if ref.get(paddingMode) == 0 then
                                imger.padding_xy(imgDst, 
                                    ref.get(paddingKernelWidth), ref.get(paddingKernelHeight), 
                                    ref.get(paddingKernelAX), ref.get(paddingKernelAY), 
                                    ref.get(paddingBorder), ref.get(paddingGray)
                                )
                            else
                                imger.padding_size(imgDst, 
                                    ref.get(paddingTop), ref.get(paddingBottom), 
                                    ref.get(paddingLeft), ref.get(paddingRight), 
                                    ref.get(paddingBorder), ref.get(paddingGray)
                                )
                            end
                        end
                        ...
                        if ref.get(grayscaleWeightedEnabled) then
                            imger.gray_add_weighted(imgDst, imgSrc, 
                                ref.get(grayscaleWeight1), ref.get(grayscaleWeight2)
                            )
                        end
                        ...
                        if ref.get(effectPixelateEnabled) then
                            imger.pixelate(imgDst, 
                                ref.get(effectPixelateSize), ref.get(effectPixelateGray)
                            )
                        end
                        ...
                        if ref.get(blurBoxEnabled) then
                            ...
                            imger.blur_box_xy(imgDst, width, height, ax, ay, 
                                ref.get(blurBoxBorder), ref.get(blurBoxGray)
                            )
                        end

                        if ref.get(gaussEnabled) then
                            imger.blur_gaussian(imgDst, ref.get(gaussRadius), ref.get(gaussSigma), 
                                ref.get(gaussBorder), ref.get(gaussGray)
                            )
                        end
                        ...
                        if ref.get(thresholdEnabled) then
                            if ref.get(thresholdMode) == 0 then
                                ...
                                imger.threshold(imgDst, t, ref.get(thresholdMethod), 
                                    ref.get(thresholdTo16)
                                )
                            else
                        ...
                        if ref.get(edgeCannyEnabled) then
                            imger.edge_canny_inplace(imgDst, 
                                ref.get(edgeCannyLower), ref.get(edgeCannyUpper), 
                                ref.get(edgeCannyKernel), ref.get(edgeCannyGray)
                            )
                        end
                        ...
                        if ref.get(edgeSobelEnabled) then
                            ...
                            if dir == 0 then
                                imger.edge_sobel_inplace(imgDst, 
                                    ref.get(edgeSobelBorder), ref.get(edgeSobelGray)
                                )
                            elseif dir == 1 then
                                imger.edge_sobel_horizontal_inplace(imgDst, 
                                    ref.get(edgeSobelBorder), ref.get(edgeSobelGray)
                                )
                            elseif dir == 2 then
                                imger.edge_sobel_vertical_inplace(imgDst, 
                                    ref.get(edgeSobelBorder), ref.get(edgeSobelGray)
                                )
                            end
                        end
                        ...
                        if ref.get(edgeLaplacianEnabled) then
                            imger.edge_laplacian_inplace(imgDst, 
                                ref.get(edgeLaplacianKernel), ref.get(edgeLaplacianBorder), 
                                ref.get(edgeLaplacianGray)
                            )
                        end

                        if ref.get(rotateEnabled) then
                            imger.rotate_xy(imgDst, ref.get(rotateAngle), 
                                ref.get(rotateAX), ref.get(rotateAY),
                                ref.get(rotateResize), ref.get(rotateGray)
                            )
                        end

                        if ref.get(resizeEnabled) then
                            imger.resize(imgDst, 
                                ref.get(resizeScalex), ref.get(resizeScaley), 
                                ref.get(resizeInterp), ref.get(resizeGray)
                            )
                        end
            ...
            wg_filter("Source", {
                gui.wg_row({
                    ...
                    gui.wg_button_radio("None", ref.get(histogramMode) == 0)
                        :on_change(function()
                            ref.set(histogramMode, 0)
                        end),
                    gui.wg_button_radio("Image", ref.get(histogramMode) == 1)
                        :on_change(function()
                            ref.set(histogramMode, 1)
                        end),
                    gui.wg_button_radio("Plot", ref.get(histogramMode) == 2)
                        :on_change(function()
                    ...
                    gui.wg_button("Refresh Source")
                        ...
                        :on_click(function()
                            ...
                            elseif ref.get(srcOption) == 1 then
                                imgSrc = imger.gradient_linear_xy(200, 200,
                                    ref.get(srcGradientStart), ref.get(srcGradientEnd), 
                                    ref.get(srcGradientDir), "src", image.ENCODING_PNG
                                )
                            elseif ref.get(srcOption) == 2 then
                                imgSrc = imger.gradient_sigmoidal_xy(200, 200,
                                    ref.get(srcGradientStart), ref.get(srcGradientEnd), 
                                    ref.get(srcGradientDir), "src", image.ENCODING_PNG
                                )
                            elseif ref.get(srcOption) == 3 then
                                imgSrc = noise.simplex_image_new(ref.get(srcNoiseSeed),
                                    ref.get(srcNoiseScale), ref.get(srcNormalize), 
                                    "src", image.ENCODING_PNG,
                                    200, 200, image.MODEL_RGBA,
                                    false, true
                                )
                            elseif ref.get(srcOption) == 4 then
                                imgSrc = image.new_random("src", image.ENCODING_PNG, 
                                    200, 200, 
                                    false, image.MODEL_RGBA
                                )
                            end
                ...
                gui.wg_row({
                    ...
                    gui.wg_row({
                        gui.wg_button_radio("Horizontal", ref.get(srcGradientDir) == 0)
                            :on_change(function()
                                ref.set(srcGradientDir, 0)
                            end),
                        gui.wg_button_radio("Vertical", ref.get(srcGradientDir) == 1)
                            :on_change(function()
            ...
            wg_filter("Grayscale", {
                ...
                gui.wg_row({
                    ...
                    gui.wg_dummy(gui.calc_text_size_width("Add Weighted") - gui.calc_text_size_width("Scalar") - spx, 1),
                    gui.wg_slider_int(grayscaleScalar, 0, 255):size(250 + gui.calc_text_size_width("Weight 1") + gui.calc_text_size_width("Weight 2") + (spx + (isx*2))),
                    gui.wg_button_arrow(gui.DIR_LEFT)
                        :on_click(function()
            ...
            wg_filter("Edge Detection", {
                ...
                gui.wg_row({
                    gui.wg_button_radio("Both", ref.get(edgeSobelDir) == 0)
                        :on_change(function()
                            ref.set(edgeSobelDir, 0)
                        end),
                    gui.wg_button_radio("Horizontal", ref.get(edgeSobelDir) == 1)
                        :on_change(function()
                            ref.set(edgeSobelDir, 1)
                        end),
                    gui.wg_button_radio("Vertical", ref.get(edgeSobelDir) == 2)
                        :on_change(function()
            ...
            wg_filter("Padding", {
                gui.wg_row({
                    ...
                    gui.wg_button_radio("XY", ref.get(paddingMode) == 0)
                        :on_change(function()
                            ref.set(paddingMode, 0)
                        end),
                    gui.wg_button_radio("Size", ref.get(paddingMode) == 1)
                        :on_change(function()
            ...
            wg_filter("Threshold", {
                gui.wg_row({
                    ...
                    gui.wg_button_radio("Normal", ref.get(thresholdMode) == 0)
                        :on_change(function()
                            ref.set(thresholdMode, 0)
                        end),
                    gui.wg_button_radio("Otsu", ref.get(thresholdMode) == 1)
                        :on_change(function()
